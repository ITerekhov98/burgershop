...
## Быстрый запуск/обновление

Для обновления кодовой базы и перезапуска всех сервисов воспользуйтесь специальным bash-скриптом. Для этого убедитесь что в `.env` указаны все необходимые переменные (для prod-версии), перейдите в каталог проекта и выполните команду:
